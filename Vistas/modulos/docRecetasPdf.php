...
INNER JOIN pacientes as p ON r.idpaciente=p.idpaciente
    ...
    $pdf->AddPage();
     ...
     if(empty($datos["doctornombre"])){
         $pdf = new text();
         $pdf->AddPage();
        $pdf->SetTitle("Doctor sin recetas");
         $pdf->SetFont("Arial", "B", 20);
         $pdf->Cell(290, 0, "Ups! :(", 0, 1, "C");
        $pdf->Ln(20);
        $pdf->Cell(290, 0, "Este doctor todavía no ha generado recetas", 0, 1, "C");
...
$apellido=$datos['doctorapellido'];
            $pdf->AliasNbPages();
      $pdf->SetTitle("Reporte de recetas de Dr. ".$nombredoct);
      $pdf->SetFont("Arial", "B", 20);
    $pdf->Cell(290, 0, 'Todas las recetas del Dr '.$nombredoct. ' ' .$apellido, 0, 1, "C");
    $pdf->Ln(5);
